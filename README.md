...
<img alt="Control Flow Graph" src="https://app.code2flow.com/Odmx0A.svg" width="75%" />
...
**JaCoCo** тоже с нами согласен (Cxty == 2), значит только для этого метода (а всего таких пять) придется написать _два_ unit-теста, чтобы обеспечить 100% покрытие по счетчику `BRANCHES` (ветвление).
...
Кроме того, согласно [многим практическим исследованиям](https://www.sciencedirect.com/topics/computer-science/cyclomatic-complexity) ЦС сама по себе не добавляет новой информации о сложности кода (предсказумое количество ошибок) по сравнению с общим количеством строк кода:
